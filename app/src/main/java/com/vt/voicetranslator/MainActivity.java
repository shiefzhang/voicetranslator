...
public final class MainActivity extends Activity {
    ...
    private ClickableSpan rowSelector(int targetPage,String key){return new ClickableSpan(){public void onClick(View widget){focusedKeys[targetPage]=key;scrollSelectionRequested[targetPage]=true;update();}public void updateDrawState(TextPaint paint){paint.setColor(ink);paint.setUnderlineText(false);}};}
    ...
    private void start(){
        ...
        final int targetPage=page;final long id=++session;focusedKeys[targetPage]="";
        pipeline=new Pipeline(asr,mt,src(page),dst(page),prefs.getInt("pause",800),new Pipeline.Listener(){
            ...
            public void done(){ui(()->{pipeline=null;if(!status.startsWith("错误")&&!status.startsWith("录音错误"))status="录音已停止 · 点击开始新的翻译";render();});}
    ...
    private void modelRow(LinearLayout l,String title,String kind){LinearLayout row=new LinearLayout(this);row.setPadding(dp(24),dp(16),dp(24),dp(16));row.setGravity(Gravity.CENTER_VERTICAL);LinearLayout labels=vertical();labels.addView(text(title,18,ink));TextView name=text(prefs.getString(kind+"Name","未导入模型包"),12,muted);name.setPadding(0,dp(6),dp(8),0);labels.addView(name);row.addView(labels,new LinearLayout.LayoutParams(0,-2,1));Button pick=button("选择模型包",false);row.addView(pick,new LinearLayout.LayoutParams(dp(124),dp(48)));pick.setEnabled(!importing);pick.setOnClickListener(v->{if(pipeline!=null){message("正在处理之前的录音，请完成后再导入模型包。");return;}importKind=kind;Intent in=new Intent(Intent.ACTION_OPEN_DOCUMENT);in.addCategory(Intent.CATEGORY_OPENABLE);in.setType("*/*");startActivityForResult(in,60);});l.addView(row);divider(l);}
    @Override protected void onActivityResult(int request,int result,Intent data){super.onActivityResult(request,result,data);if(request==60&&result==RESULT_OK&&data!=null&&data.getData()!=null){Uri uri=data.getData();String kind=importKind;importing=true;ProgressDialog dialog=new ProgressDialog(this);dialog.setTitle("导入模型包");dialog.setMessage("正在读取…");dialog.setCancelable(false);dialog.show();new Thread(()->{try{if(kind.equals("translation"))LocalLlm.close();ModelPackages.install(this,uri,kind,s->ui(()->dialog.setMessage(s)));ui(()->{importing=false;dialog.dismiss();status="模型包已导入";render();});}catch(Exception e){ui(()->{importing=false;dialog.dismiss();render();message("导入失败："+e.getMessage());});}},"vt-import").start();}}
